...
class SculptorFitter:
    def __init__(
        ...
        try:
            ...
            print(
                f"Error initializing SCULPTOR_layer (possibly related to '{sculptor_paradict_path}'): {e}"
            )
            return
    ...
    def get_sculptor_mesh(self) -> trimesh.Trimesh:
        vertices = self.sculptor_model.template_face
        faces = self.sculptor_model.facialmesh_face

        temp_vertices_index = faces[:, 0].reshape(faces.shape[0], 1)
        additional_meshes = np.concatenate([temp_vertices_index, faces[:, 2:]], axis=1)
        faces = np.concatenate([faces[:, 0:3], additional_meshes], axis=0)
    ...
    def compute_approximate_scale(self) -> np.ndarray:
        ...
        template_vertices = torch.tensor(
            self.sculptor_model.template_face,
            dtype=self.sculptor_model.dtype,
            device=self.device,
        )
        ...
        target_vertices_tensor = torch.tensor(
            self.target_vertices, dtype=self.sculptor_model.dtype, device=self.device
        )

        scale = torch.tensor(
            1.0, dtype=self.sculptor_model.dtype, requires_grad=True, device=self.device
        )
        eye = torch.tensor(
            np.eye(3),
            dtype=self.sculptor_model.dtype,
    ...
    def compute_initial_translation_and_rotation(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        """
        template_vertices: torch.Tensor = torch.tensor(
            self.sculptor_model.template_face,
            dtype=self.sculptor_model.dtype,
            device=self.device,
        )
        ...
        translation = self.initial_translation.clone().requires_grad_(True)
        rotation = self.initial_rotation.clone().requires_grad_(False)

        optimizer = torch.optim.AdamW([translation], lr=self.lr)
        ...
        for iter_num in pbar:
            ...
            loss = self.custom_chamfer_distance(
                fitted_mesh_vertices, target_vertices_tensor
            )
    ...
    def get_trimesh(
        self, skin_vertices: torch.Tensor, skull_vertices: torch.Tensor
        ...
        additional_meshes = np.concatenate(
            [temp_vertices_index, skin_faces[:, 2:]], axis=1
        )
    ...
    def custom_chamfer_distance(
        ...
        if p1_lengths is not None:
            mask_p1 = (
                torch.arange(p1.shape[1], device=p1.device)[None, :]
                >= p1_lengths[:, None]
            )  # (B, N)
            dist_sq.masked_fill_(mask_p1[:, :, None].expand_as(dist_sq), float("inf"))
        if p2_lengths is not None:
            mask_p2 = (
                torch.arange(p2.shape[1], device=p2.device)[None, :]
                >= p2_lengths[:, None]
            )  # (B, M)
            dist_sq.masked_fill_(mask_p2[:, None, :].expand_as(dist_sq), float("inf"))

        dist_p1_p2, _ = torch.min(
            dist_sq, dim=2
        )  # (B, N), min dist from each point in p1 to p2
        dist_p2_p1, _ = torch.min(
            dist_sq, dim=1
        )  # (B, M), min dist from each point in p2 to p1

        # Mask invalid points before mean
        if p1_lengths is not None:
            dist_p1_p2.masked_fill_(
                mask_p1, 0.0
            )  # Fill with 0, sum then divide by actual length
        ...
        if p1.shape[1] == 0:  # only p2 has points
            return (
                torch.mean(loss_p2_p1)
                if p2_lengths is None
                else torch.mean(loss_p2_p1[p2_lengths > 0])
            )
        if p2.shape[1] == 0:  # only p1 has points
            return (
                torch.mean(loss_p1_p2)
                if p1_lengths is None
                else torch.mean(loss_p1_p2[p1_lengths > 0])
            )
    ...
    def fit(self) -> tuple[dict[str, np.ndarray], torch.Tensor]:
        for i in range(4):
            ...
            scale_fitted_mesh.export(
                os.path.join(self.output_dir, f"scale_fitted_mesh_{i}.ply")
            )
            print(f"Stage {i + 1} Scale: {scale}")

            print("Fit Translations and Rotations...")
            fitted_translation, fitted_rotation = (
                self.compute_initial_translation_and_rotation()
            )
            print(f"Stage {i + 1} translation: {fitted_translation}")
            print(f"Stage {i + 1} rotation: {fitted_rotation}")
            self.initial_translation = torch.tensor(
                fitted_translation, dtype=self.sculptor_model.dtype, device=self.device
            )
            self.initial_rotation = torch.tensor(
                fitted_rotation, dtype=self.sculptor_model.dtype, device=self.device
            )

            transformed_vertices: torch.Tensor = torch.tensor(
                self.sculptor_model.template_face,
                dtype=self.sculptor_model.dtype,
                device=self.device,
            )
        ...
        beta_s = torch.tensor(
            np.zeros((1, self.n_shape)) + 1e-5,
            dtype=self.sculptor_model.dtype,
            requires_grad=True,
        )
        pose_theta = torch.tensor(
            np.tile(
                np.zeros(3 * (self.num_joints + 1)).reshape(
                    1, 3 * (self.num_joints + 1)
                ),
                (1, 1),
            ),
            dtype=self.sculptor_model.dtype,
        ...
        jaw_offset = torch.tensor(
            np.zeros((1, 3, 1)), dtype=self.sculptor_model.dtype, requires_grad=True
        )
        ...
        for iter_num in pbar:
            ...
            loss_chamfer: torch.Tensor = self.custom_chamfer_distance(
                fitted_mesh_vertices, self.target_vertices
            )

            loss_reg_shape: torch.Tensor = torch.mean(torch.square(beta_s))
            loss_reg_pose: torch.Tensor = torch.mean(
                torch.square(pose_theta)
            )  # Regularize pose parameters

            loss = (
                loss_chamfer
                + self.shape_reg_weight * loss_reg_shape
                + self.pose_reg_weight * loss_reg_pose
            )
